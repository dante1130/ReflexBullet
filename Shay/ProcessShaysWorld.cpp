...
void PSW::RenderLoop(int val)
{
	if (ActiveGameWorld)
	{
		GM::GameInit(width, height);
		return;
	}
...
void PSW::reshape(int w, int h)
{
	...
	glLoadIdentity();
	glViewport(0, 0, w, h);
	gluPerspective(60, ratio, 1, far);
	glMatrixMode(GL_MODELVIEW);
}

void PSW::GoToGameWorld()
{
	...
	if (far < 100)
	{
		ActiveGameWorld = true;
	}
}

//--------------------------------------------------------------------------------------
void PSW::keys(unsigned char key, int x, int y)
{
	switch (key)
	{
	case 'T':
...
//--------------------------------------------------------------------------------------
void PSW::releaseKeys(unsigned char key, int x, int y)
{
	switch (key)
	{
		// step left or right
...
//  Mouse Buttons
//--------------------------------------------------------------------------------------
void PSW::Mouse(int button, int state, int x, int y)
{
...
//--------------------------------------------------------------------------------------
void PSW::mouseMove(int x, int y)
{
	cam.MouseMove(x, y);
...
void PSW::CreateBoundingBoxesPillar()
{
...
void PSW::CreateBoundingBoxesExtendedArea()
{
	//Gaming hub
...
void PSW::CreatePlainsExtendedArea()
{
	//						x1, x2				, y1, y2		, z1, z2
